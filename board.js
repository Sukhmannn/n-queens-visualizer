...
export function board(SIZE) {
    ...
    container.style.setProperty("--Ogrid-rows", SIZE);
    container.style.setProperty("--Ogrid-cols",SIZE);
    for(let j=0; j<SIZE * SIZE; j++) {
        let cell = document.createElement("div");
        let item = document.createElement("i");
        cell.appendChild(item).className = "fas fa-crown";
        container.appendChild(cell).className = "Ogrid-item q-array-title";
    }
    const arrayTiles = document.getElementByClassName("q-array-title");
    const crownsI= document.getElementByClassName("fa-corwn");
